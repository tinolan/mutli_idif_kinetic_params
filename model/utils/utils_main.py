...
def apply_final_activation(logits, config):
  ...
  if config["final_activation"] == "auto": 
    ...
    kinetic_params[:, 4, :, :, :] = torch.clamp(logits[:, 4, :, :, :].to(machine), 
                                                config["multi_clamp_params"]["alpha"][0], config["multi_clamp_params"]["alpha"][1]) 
    kinetic_params[:, 5, :, :, :] = torch.clamp(logits[:, 5, :, :, :].to(machine),
                                                config["multi_clamp_params"]["beta"][0], config["multi_clamp_params"]["beta"][1])
...
def make_save_folder_struct(current_run_name, root_checkpoints_path, trainer_ckpt_path, if_return_only=False):
  run_name = current_run_name
  
  if not trainer_ckpt_path:
        raise ValueError("The checkpoint path is not provided or is None.")
