...
labels_to_organ = {
    0: "background", 3: "liver", 4: "lung_1", 5: "lung_2", 6: "kidney_1", 7: "kidney_2",
    10: "heart", 11: "aorta", 13: "spleen", 16: "unknown", 19: "bones"
}

def log_slice(config, PET_measurements, K_prediction):
    b = PET_measurements.shape[0]
    fig, ax = plt.subplots(nrows=b, ncols=9, figsize=(4 * 9, 4 * b))  # Increased ncols to 10 for k4, alpha and beta
    vmax_list = [10, 10, 1, 1, 1, 1, 4]
    if b == 1:
        ...
        im_2 = ax[8].imshow(K_slice.to("cpu").detach().numpy(), cmap=cmap, vmin=0, vmax=vmax_list[6])
        ...
        ax[4].set_title("k3")
        ax[5].set_title("Vb")
        ax[6].set_title("alpha")
        ax[7].set_title("beta")
        ax[8].set_title("K1/k2")
        plt.colorbar(im_2, ax=ax[8], fraction=0.046, pad=0.04)
        #Berechnung von 1 - alpha
        """alpha = K_prediction[0, config["output_size"]-1, 0, :, :]
        one_minus_alpha = 1 - alpha    
        im_3 = ax[7].imshow(one_minus_alpha.to("cpu").detach().numpy(), cmap=cmap, vmin=0, vmax=vmax_list[5])
        plt.colorbar(im_3, ax=ax[7], fraction=0.046, pad=0.04)"""
    else:
        for j in range(b):
            ...
            im_2 = ax[j, 8].imshow(K_slice.to("cpu").detach().numpy(), cmap=cmap, vmin=0, vmax=vmax_list[6])
            plt.colorbar(im_2, ax=ax[j, 8], fraction=0.046, pad=0.04)
        ...
        ax[0, 4].set_title("k3")
        ax[0, 5].set_title("Vb")
        ax[0, 6].set_title("alpha")
        ax[0, 7].set_title("beta")
        ax[0, 8].set_title("K1/k2")
...
def mask_data(test, TAC_mes_batch, TAC_pred_batch, y_pred, time_stamp, patch_size):
    """
    This function masks (i.e., sets to zero) all the pixels where the Area Under the Curve (AUC)
    of the TAC is lower than a threshold (empirically set to 10).
    """
    ...
    test = torch.multiply(test, mask)

    # Prepare predictions
    mask = maskk.repeat(1, 6, 1, 1, 1)  # alpha and beta
    y_pred = torch.multiply(y_pred, mask)

    return test, TAC_mes_batch, TAC_pred_batch, y_pred
