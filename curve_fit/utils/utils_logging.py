...
def log_slice(config, PET_measurements, K_prediction):
    ...
    if b == 1:
        ...
        for j in range(b):
            ...
            im_2 = ax[j, 7].imshow(K_slice.to("cpu").detach().numpy(), cmap=cmap, vmin=0, vmax=vmax_list[6])
            plt.colorbar(im_2, ax=ax[j, 7], fraction=0.046, pad=0.04)
            alpha = K_prediction[0, config["output_size"]-1, 0, :, :]
            one_minus_alpha = 1 - alpha    
            im_3 = ax[7].imshow(one_minus_alpha.to("cpu").detach().numpy(), cmap=cmap, vmin=0, vmax=vmax_list[5])
            plt.colorbar(im_3, ax=ax[7], fraction=0.046, pad=0.04)
...
def log_curves(PET_measurements, PET_prediction, t, time_stamp, current_epoch):
  b = PET_measurements.shape[0]
  fig, ax = plt.subplots(nrows=b, ncols=10, figsize=(2*10, 2*b))
  if b == 1:
    for i in range(5):  
...
def mask_data(TAC_mes_batch, TAC_pred_batch, y_pred, time_stamp, patch_size):
    ...
    # Prepare predictions (including k4)
    mask = maskk.repeat(1, 5, 1, 1, 1)  # Updated to include k4, alpha and beta
