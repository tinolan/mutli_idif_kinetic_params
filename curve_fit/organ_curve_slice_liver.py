...
def process_pet_slice(patient, slice_idx, slice_orientation="axial"):
    ...
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "voxel", f"liver_Vb_{slice_orientation}_{patient}_slice_{slice_idx}")
...
def process_pet_slice_normal(patient, slice_idx, slice_orientation="axial"):
    print(f"whole slice for aorta input {patient}")
    ...
    label_map_path = glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/labels.nii.gz")[0]
    PET_list = natsorted(glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/PET_*.nii.gz"))
    save_path = os.path.join("/home/guests/valentin_langer/data/plots/organ_curve", "voxel", f"liver_normal_{slice_orientation}_{patient}_slice_{slice_idx}")
    os.makedirs(save_path, exist_ok=True)
...
if __name__ == '__main__':
    process_pet_slice(patient="06", slice_idx=230, slice_orientation="axial")
    process_pet_slice_normal(patient="06", slice_idx=230, slice_orientation="axial")
