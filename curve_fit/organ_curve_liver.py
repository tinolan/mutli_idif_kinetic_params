...
def process_pet_liver(patient):
    ...
    save_path = os.path.join("/xxx/xxx/xxx/data/plots/organ_curve", "image-derived", f"liver_Vb_{patient}")
...
def process_pet_liver_normal(patient):
    ...
    label_map_path = glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/labels.nii.gz")[0]
    PET_list = natsorted(glob.glob(root_data_path + f"/DynamicPET/*DynamicFDG_{patient}/NIFTY/Resampled/PET_*.nii.gz"))
    save_path = os.path.join("/xxx/xxx/xxx/data/plots/organ_curve", "image-derived", f"liver_normal_{patient}")
    os.makedirs(save_path, exist_ok=True)
